...
typedef enum _X4MONITORSTATE
{
   X4MonitorOn,
   X4MonitorOff,
} X4MONITORSTATE, *PX4MONITORSTATE;

/******************************************************************************/

typedef struct _X4CALLBACK
{
   unsigned long  Size;
   unsigned long TBD;
}  X4CALLBACK, *PX4CALLBACK;


typedef void (X4CBKAPI X4CALLBACKFN) (
   HX4DLL      hDevice,
   X4CALLBACK  callbackData,
   ULONG_PTR   userData );
typedef X4CALLBACKFN *PX4CALLBACKFN;

/******************************************************************************/

typedef struct _X4DEVICECALLBACK
{
   unsigned long TBD;
}  X4DEVICECALLBACK, *PX4DEVICECALLBACK;


typedef void (X4CBKAPI X4DEVICECALLBACKFN) (
   HX4DEVICE         hDevice,
   X4DEVICECALLBACK  callbackData,
   ULONG_PTR         userData );
typedef X4DEVICECALLBACKFN *PX4DEVICECALLBACKFN;
